...
class WalletTopUpAdmin(admin.ModelAdmin):
    list_display = (
        'public_id', 'wallet', 'amount', 'status', 'gateway_authority',
        'gateway_ref_id', 'created_at', 'credited_at',
    )
    list_filter = ('status', 'created_at')
    search_fields = ('public_id', 'idempotency_key', 'gateway_authority', 'wallet__user__email')
    readonly_fields = (
        'public_id', 'wallet', 'amount', 'status', 'idempotency_key',
